...
def compute_node_reward(
    ...
    decompse_mask: torch.Tensor,
    num_nodes: torch.Tensor,
    decompose_factor: float,
    theta: float
):
    from recurrent.utils import final_tensor

    num_nodes = final_tensor(num_nodes, final_mask, sample_index)
    reward_score = token_level_rewards.sum(dim=-1)  # [conv_size]
    
    bsz = token_level_rewards.size(0)
    nodes_reward = torch.zeros(bsz, dtype=torch.float32, device=token_level_rewards.device)
    
    is_correct = reward_score > theta
    
    with torch.no_grad():
        unique_indices = np.unique(index)
        for uid in unique_indices:
            group_mask = torch.tensor(index == uid, device=token_level_rewards.device, dtype=torch.bool)
            correct_mask = group_mask & is_correct
            
            num_correct = correct_mask.sum().item()
            if num_correct > 1:
                correct_nodes = num_nodes[correct_mask].float()
                min_nodes = correct_nodes.min()
                max_nodes = correct_nodes.max()
                if max_nodes > min_nodes:
                    norm_reward = - (correct_nodes - min_nodes) / (max_nodes - min_nodes)
                    nodes_reward[correct_mask] = norm_reward
                else:
                    nodes_reward[correct_mask] = 0.0
            
    nodes_reward_scaled = nodes_reward * decompose_factor
    nodes_reward_per_token = nodes_reward_scaled.unsqueeze(-1) / token_level_rewards.size(-1)
    
    new_token_level_rewards = token_level_rewards + nodes_reward_per_token.tile(1, token_level_rewards.size(-1))
    
    return new_token_level_rewards, nodes_reward
...
def compute_1D_ReA_advantage(token_level_rewards: torch.Tensor, num_nodes: torch.Tensor,
                              index: torch.Tensor,
                              final_mask: torch.Tensor,
                              sample_index: torch.Tensor,
                              epsilon: float = 1e-6,
    ...
    Returns:
        advantages: (torch.Tensor) shape: (bsz,)
    """
    from recurrent.utils import final_tensor  
    scores = token_level_rewards.sum(dim=-1)
    num_nodes = final_tensor(num_nodes, final_mask, sample_index)

    correct_mask = (scores > theta)
    ...
    def _norm(x):
        y = (-x).exp()
        return y / (y.sum() + epsilon)
    
    unique_ids = np.unique(index)
    for uid in unique_ids:
        group_idx = torch.tensor(index == uid).nonzero(as_tuple=True)[0]
        
        group_correct = correct_mask[group_idx].long()
        group_nodes = num_nodes[group_idx]
        
        is_all_correct = (group_correct.sum().item() == len(group_correct))
        is_all_wrong = (group_correct.sum().item() == 0)
        
        if is_all_correct:
            if len(set(group_nodes.tolist())) == 1:
                group_adv = torch.zeros(len(group_correct), device=device)
            else:
                tensor_ns = -group_nodes.float()

                std_val = tensor_ns.std() + epsilon
                group_adv = (tensor_ns - tensor_ns.mean()) / std_val
        elif is_all_wrong:
            if len(set(group_nodes.tolist())) == 1:
                group_adv = torch.zeros(len(group_correct), device=device)
            else:
            ...
            msk = group_correct.bool()
            ns = group_nodes.float()
            cor = ns[msk]
            wro = ns[~msk]
            
            group_adv = torch.empty(len(group_correct), device=device)
            group_adv[msk] = _norm(cor)
            group_adv[~msk] = -_norm(wro)

        adv[group_idx] = group_adv
        
    return adv



def compute_action_rewards(all_prompt_str: list[str], all_responses_str: list[str], batch, reward_batch, sample_index: torch.Tensor, rewarded_scalar: torch.Tensor) -> torch.Tensor:
    with torch.no_grad():
        ...
        update_memory_mask = batch.batch["integration_mask"] | batch.batch["read_mask"] | batch.batch["final_mask"]

        decompse_repeats = batch.batch["repeats"] & batch.batch["decompose_mask"]

        for prompt_str, response_str, action_type, is_repeat, reward_batch_item in zip(all_prompt_str, all_responses_str, update_memory_mask, decompse_repeats, reward_batch[sample_index]):
            action_type = int(action_type)
            ground_truth_list = reward_batch_item.non_tensor_batch['reward_model']['ground_truth']
            if not action_type: # callback: min_callback as golden standard, calculate penalty for over-callback
                reward_score = 0.0
            else: # memory: word-level recall reward
                ...
                reward_score = generated_memory_recall - previous_memory_recall
...
def compute_action_metrics(batch):
    
    action_reward = batch.batch["state_scores"]

    metrics = {
        "critic/R-action/mean": torch.mean(action_reward).detach().item(),
        "critic/R-action/max": torch.max(action_reward).detach().item(),
        "critic/R-action/min": torch.min(action_reward).detach().item(),
    }
...
def compute_custom_metrics(
    ...
    sequence_score = batch.batch["score_token_level_scores"].sum(-1)
    node_score = batch.batch["score_nodes"]


    assert sequence_score.shape == nnodes.shape, \
    f"Sequence_score's Shaps: {sequence_score.shape}, Nnodes's Shape: {nnodes.shape}"

    nnodes = nnodes.float()

    true_nnodes = nnodes[sequence_score > theta]
    false_nnodes = nnodes[sequence_score <= theta]

    if not true_nnodes.numel():
        true_nnodes = torch.tensor([0.0])

    if not false_nnodes.numel():
        false_nnodes = torch.tensor([0.0])

    metrics = {
        "critic/R-score/mean": torch.mean(sequence_score).detach().item(),
        "critic/R-score/max": torch.max(sequence_score).detach().item(),
        "critic/R-score/min": torch.min(sequence_score).detach().item(),
        "critic/R-nodes/mean": torch.mean(node_score).detach().item(),
        "critic/R-nodes/max": torch.max(node_score).detach().item(),
        "critic/R-nodes/min": torch.min(node_score).detach().item(),

        "critic/nodes-cnt-all/mean": torch.mean(nnodes).detach().item(),
        "critic/nodes-cnt-all/max": torch.max(nnodes).detach().item(),
        "critic/nodes-cnt-all/min": torch.min(nnodes).detach().item(),

        "critic/nodes-cnt-true/mean": torch.mean(true_nnodes).detach().item(),
        "critic/nodes-cnt-true/max": torch.max(true_nnodes).detach().item(),
        "critic/nodes-cnt-true/min": torch.min(true_nnodes).detach().item(),

        "critic/nodes-cnt-false/mean": torch.mean(false_nnodes).detach().item(),
        "critic/nodes-cnt-false/max": torch.max(false_nnodes).detach().item(),
        "critic/nodes-cnt-false/min": torch.min(false_nnodes).detach().item(),
    }
...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        enabled_conf = config.recursive.memory
        ...
        assert hasattr(enabled_conf, "async_path") and hasattr(enabled_conf, "async_name"), \
            f"`async_path` and `async_name` must be set in recurisve.{enabled_conf} when using async rollout"
        
        self.rregister = RRegister.from_filename(enabled_conf.async_path, enabled_conf.async_name)

        conf = dict(enabled_conf.config) if enabled_conf.config is not None else {}
        self.recurrent_config = self.rregister.config_cls(**conf)
        print("[validate_config] All configuration checks passed successfully!")

    def _create_dataloader(self):
        # TODO: we have to make sure the batch size is divisible by the dp size
        self.train_dataset = self.rregister.dataset_cls(
        ...
        self.train_dataloader = StatefulDataLoader(dataset=self.train_dataset,
                                                   ...
                                                   sampler=sampler)
        self.val_dataset = self.rregister.dataset_cls(
            recurrent_config=self.recurrent_config,
            data_config=self.config.data,
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.config.recurrent.enable:
                ...
                output_gen_batch, final_mask, read_mask, decompose_mask, integration_mask, nodes, repeats, pos_uids, type_uids, sample_index = self.generation_manager.run_llm_loop(test_gen_batch, {})
    ...
    def init_workers(self):
        ...
        from recursive.async_generation_manager import AsyncLLMGenerationManager
        self.generation_manager = AsyncLLMGenerationManager(
            ...
            agent_cls=self.rregister.agent_cls,
        )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                        ...
                        gen_batch_output, final_mask, read_mask, decompose_mask, integration_mask, nodes, repeats, pos_uids, type_uids, sample_index = self.generation_manager.run_llm_loop(gen_batch, timing_raw)
                        ...
                        gen_batch_output.batch['read_mask'] = decompose_mask
                        gen_batch_output.batch['decompose_mask'] = decompose_mask
                        gen_batch_output.batch['integration_mask'] = integration_mask
                        gen_batch_output.batch['nodes'] = nodes
                        gen_batch_output.batch['repeats'] = repeats
                    ...
                    #######
                    if self.config.trainer.balance_batch and not self.config.recursive.enable:
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.use_rm:
                            raise NotImplementedError("RM is not implemented for recursive.")
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if not self.config.algorithm.adv_estimator == AdvantageEstimator.GRPO:
                            raise NotImplementedError("Only GRPO is implemented for recursive.")


                        score_reward_tensor = reward_tensor.clone()

                        if self.config.algorithm.use_ReA:
                            advantage_scalar = compute_1D_ReA_advantage(
                                token_level_rewards=reward_tensor, num_nodes = nodes,
                                index=reward_batch.non_tensor_batch['uid'],
                                final_mask = final_mask, sample_index = sample_index,
                                theta = self.config.custom_reward.theta,
                                use_adv=self.config.algorithm.grpo_use_adv)
                            advantage_scalar = advantage_scalar[sample_index]
                            # advantage_scalar[repeats.bool() & read_mask.bool()] = 0
                            # advantage_scalar[repeats.bool() & decompose_mask.bool()] = -1
                            advantage_scalar = advantage_scalar * (1 - self.config.custom_reward.alpha)                            
                            batch.batch["score_nodes"] = advantage_scalar

                            if self.config.custom_reward.alpha > 0:
                                
                                scores_scalar = score_reward_tensor.sum(dim = -1)[sample_index].detach()
                                ...
                                batch.batch['rewards_action'] = compute_action_rewards(all_prompt_str, all_responses_str, batch, reward_batch, sample_index, scores_scalar) # [num_actions]

                                state_reward_scalar = batch.batch['rewards_action']

                                uid_indexed = reward_batch.non_tensor_batch['uid'][sample_index.cpu().numpy()] # [num_actions], str
                                step_uid_indexed = [str(uid)+str(tp_id) for uid, tp_id in zip(uid_indexed, type_uids)] # [num_actions]. for each step in each question, we have a unique uid.
                                ...
                                advantage_scalar += self.config.custom_reward.alpha * state_advantage_scalar

                                batch.batch["state_scores"] = state_reward_scalar

                        else:
                            
                            # if self.config.custom_reward.alpha == 1.0:
                            reward_tensor, nodes_reward_tensor = compute_node_reward(
                                reward_tensor, reward_batch.non_tensor_batch['uid'], 
                                final_mask, sample_index, decompose_mask, nodes, self.config.custom_reward.n_factor, self.config.custom_reward.theta)

                            advantage_scalar = compute_1D_grpo_advantage(
                                token_level_rewards=reward_tensor, 
                                index=reward_batch.non_tensor_batch['uid'],
                                use_adv=self.config.algorithm.grpo_use_adv)
                            advantage_scalar = advantage_scalar[sample_index] * (1 - self.config.custom_reward.alpha)
                            
                            batch.batch["score_nodes"] = nodes_reward_tensor[sample_index]
                            
                        if self.config.custom_reward.alpha > 0:
                            scores_scalar = score_reward_tensor.sum(dim = -1)[sample_index]
                            ...
                            uid_indexed = reward_batch.non_tensor_batch['uid'][sample_index.cpu().numpy()] # [num_actions], str
                            step_uid_indexed = [str(uid)+str(pos_id) for uid, pos_id in zip(uid_indexed, pos_uids)] # [num_actions]. for each step in each question, we have a unique uid.
                            ...
                            advantage_scalar += self.config.custom_reward.alpha * state_advantage_scalar * read_mask.long()
                        ...
                        batch.batch["score_token_level_scores"] = score_reward_tensor[sample_index]

                        if not self.config.actor_rollout_ref.actor.get('use_kl_loss', False):
                            raise NotImplementedError("KL penalty is not implemented for recursive.")
                ...
                metrics.update(compute_custom_metrics(batch, nodes, self.config.custom_reward.theta))
                if self.config.custom_reward.alpha > 0:
                    metrics.update(compute_action_metrics(batch))
