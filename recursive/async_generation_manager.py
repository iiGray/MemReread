...
from recursive.interface import AsyncOutput
from recurrent.async_utils import run_coroutine_in_chat_scheduler_loop, ChatCompletionProxy
from recurrent.chat_template.utils import set_chat_template
from verl.workers.rollout.async_server import AsyncLLMServerManager
from verl.trainer.ppo.ray_trainer import _timer
from recurrent .utils import create_position_ids, pad_tensor_list_to_length
...
class AsyncLLMGenerationManager:
    ...
    def run_llm_loop(self, gen_batch: DataProto, timing_raw) -> Tuple[DataProto, torch.BoolTensor, torch.LongTensor]:
        ...
        with _timer("mt_gather", timing_raw):
            ...
            final_mask = torch.cat([o.final_mask for o in gen_output_list]).bool()
            read_mask = torch.cat([o.read_mask for o in gen_output_list]).bool()
            decompose_mask = torch.cat([o.decompose_mask for o in gen_output_list]).bool()
            integration_mask = torch.cat([o.integration_mask for o in gen_output_list]).bool()
            # integtration_mask = torch.cat([o.integtration_mask for o in gen_output_list])
            nodes = torch.concat([o.nnodes for o in gen_output_list])
            repeats = torch.concat([o.repeats for o in gen_output_list])

            pos_uids = np.concatenate([o.uids_array for o in gen_output_list])

            type_uids = np.concatenate([o.type_array for o in gen_output_list])
        ...
        return gen_output, final_mask, read_mask, decompose_mask, integration_mask, nodes, repeats, pos_uids, type_uids, sample_index # pyright: ignore
    ...
    def concat_output(self, batch_list: list[dict]) -> DataProto:
        ...
        for i in range(len_starting):
            if  batch_list[0]["responses"][0][i] != starting[i]:
                print(f"Warning: Mismatch at {i}: expected {starting[i]}, got {batch_list[0]['responses'][0][i]}")
            # assert batch_list[0]["responses"][0][i] == starting[i], f"Mismatch at {i}: expected {starting[i]}, got {batch_list[0]['responses'][0][i]}"
        concated = {
